...
- `[STEP]` Add a `timeout=` value to the `requests.get()` call, and handle the resulting timeout/connection error explicitly (currently the request would just hang indefinitely with no timeout set at all).
- `[STEP]` Handle rate limiting (HTTP 429) explicitly.
